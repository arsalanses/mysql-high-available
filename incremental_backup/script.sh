...
CONTAINER_ID=mysql-high-available-mysql-master-1
...
mysql -h 127.0.0.1 -P 33306 -u$DB_USER -p$DB_PASSWORD -E --execute='FLUSH BINARY LOGS;' mysql

# get a list of all binary log files
BINLOGS=$(mysql -h 127.0.0.1 -P 33306 -u$DB_USER -p$DB_PASSWORD -E --execute='SHOW BINARY LOGS;' mysql | grep Log_name | sed -e 's/Log_name://g' -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')
...
echo $BINLOG_CURRENT | xargs -I % mysql -h 127.0.0.1 -P 33306 -u$DB_USER -p$DB_PASSWORD -E --execute='PURGE BINARY LOGS TO "%";' mysql
